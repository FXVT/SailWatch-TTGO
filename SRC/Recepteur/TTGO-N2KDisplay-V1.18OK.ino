...
#include "SL31160NB.h"  //image du bateau
...
BMA *sensor;               // Sensor Accelerometre
...
volatile byte NbMaxPage = 6;        //nb de pages affichables
...
void ToucheEcran() {
  ...
  // On en profite pour tester si on a double tapé la montres pour la mise en veille
  accelerometre ();
...
void setup() {
  ...
  while (!SerialBT.connected(10)) {
    ...
    ttgo->tft->setTextSize(1);
    ttgo->tft->setCursor(1, 170);
    ttgo->tft->println("Pas de reception");
...
// ====================================================================== PAGE SOG/TTG/ETA simple (Pas utilisée)
void DisplayPage1 () {
  Serial.println ("DisplayPage1");
  Cadres();
  DisplayLabelUnite1();
  while (ChoixPage == 1) {
    DisplayEntete();
    ToucheEcran();
    Serial.print ("ToucheDirection"); Serial.println (ToucheDirection);
    GetDatas();
    DisplayData1();
  }
}
...
// ====================================================================== PAGE VENT simple (Pas utilisée)
void DisplayPage3 () {
  Serial.println ("DisplayPage3");
  Cadres();
  DisplayLabelUnite3();
  while (ChoixPage == 3) {
    DisplayEntete();
...
// ====================================================================== AFFICHAGE LIBELLES NAVIGATION
void DisplayLabelUnite1() {
  ttgo->tft->setTextFont(2);
  ttgo->tft->setTextSize(2);
  // ttgo->tft->setFreeFont(FSSB18);
  ttgo->tft->setTextColor(TFT_GREEN, TFT_BLACK);
  ttgo->tft->setCursor(3, 55);
  ttgo->tft->println(F("SOG            kts"));
  ttgo->tft->setCursor(3, 118);
  ttgo->tft->println(F("TTG            "));
  ttgo->tft->setCursor(3, 181);
  ttgo->tft->println(F("ETA              "));
}
...
// ====================================================================== AFFICHAGE LIBELLES VENT
void DisplayLabelUnite3() {
  // EffaceLabelUnite();
  // ttgo->tft->setTextFont(2);
  ttgo->tft->setTextSize(1);
  ttgo->tft->setFreeFont(FSS12);
  ttgo->tft->setTextColor(TFT_GREEN, TFT_BLACK);
  ttgo->tft->setCursor(10, 90);
  ttgo->tft->println(F("AWA          deg"));
  ttgo->tft->setCursor(10, 155);
  ttgo->tft->println(F("TWS          kts"));
  ttgo->tft->setCursor(10, 210);
  ttgo->tft->println(F("GWD          deg"));
}
...
// ====================================================================== AFFICHAGE DONNEES NAVIGATION
/*   Alignement des points decimaux
  float v=3.14;
...
void DisplayData1() {
  ...
  ttgo->tft->fillRect(60, 50, 130, 50, TFT_BLACK);
  ttgo->tft->setCursor(90, 90);
  ttgo->tft->setTextColor(TFT_RED, TFT_BLACK);
  ttgo->tft->println(SOG, 1);
  ttgo->tft->fillRect(60, 113, 130, 50, TFT_BLACK);
  ttgo->tft->setCursor(90, 155);
  ttgo->tft->setTextColor(TFT_RED, TFT_BLACK);
  ttgo->tft->println(TTG);
  ttgo->tft->fillRect(60, 176, 130, 40, TFT_BLACK);
...
// ====================================================================== AFFICHAGE DONNEES VENT
void DisplayData3() {
  ttgo->tft->setTextFont(2);
  ttgo->tft->setTextSize(3);
  //ttgo->tft->setFreeFont(FSSB24);
  ttgo->tft->fillRect(70, 50, 110, 50, TFT_BLACK);
  ttgo->tft->setCursor(70, 90);
  ttgo->tft->setTextColor(TFT_RED, TFT_BLACK);
  ttgo->tft->println(AWA, 0);
  ttgo->tft->fillRect(70, 113, 110, 50, TFT_BLACK);
  ttgo->tft->setCursor(70, 155);
  ttgo->tft->setTextColor(TFT_RED, TFT_BLACK);
  ttgo->tft->println(TWS, 1);
  ttgo->tft->fillRect(70, 176, 110, 50, TFT_BLACK);
  ttgo->tft->setCursor(70, 210);
  ttgo->tft->setTextColor(TFT_RED, TFT_BLACK);
  ttgo->tft->println(GWD, 1);
  delay (tempo);
}
...
void parseData() {      // split the data into its parts   <SOG,TTG,ETA,BTW,DTW,CAP,TWA,AWA,TWS,AWS,GWD,SOC,VOLTAGE,AMPERAGE,TECHAP,THUILE,RPM,PROF>
  ...
  strtokIndx = strtok(NULL, ", "); // this continues where the previous call left off
  DTW = atoi (strtokIndx);     // convert this part to an integer
...
// ====================================================================== GETION DE L'ACCELEROMETRE POUR LA MIE EN VEILLE PAR DOUBLE TAPE UR L'ECRAN
void accelerometre()  //https://foxpost.xyz/pages/ttgo_watch/
{
  ...
  Serial.print(power->getBattDischargeCurrent());
  Serial.println(" mA");
}
